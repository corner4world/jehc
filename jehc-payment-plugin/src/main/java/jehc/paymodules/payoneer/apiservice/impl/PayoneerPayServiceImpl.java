package jehc.paymodules.payoneer.apiservice.impl;
...
import jehc.paymodules.payoneer.apiservice.AdvancedPayService;
import jehc.paymodules.payoneer.model.PayoneerTransactionType;

/**
 * payoneer业务逻辑
 */
public class PayoneerPayServiceImpl extends PayServiceImpl implements AdvancedPayService {
    /**
     * 测试地址
     */
    ...
    public PayoneerPayServiceImpl(PayConfigDao payConfigDao) {
        super(payConfigDao);
    }

    public PayoneerPayServiceImpl(PayConfigDao payConfigDao, HttpConfigDb configDao) {
    ...
    private HttpHeader authHeader(){
        ...
        headers.add(new BasicHeader("Authorization",  "Basic " + authorizationString(getPayConfigDao().getSeller(), getPayConfigDao().getKeyPrivate())));
    ...
    public String getReqUrl(TransactionType type) {
        return (payConfigDao.isTest() ? SANDBOX_DOMAIN : RELEASE_DOMAIN) + payConfigDao.getPid() + "/" + type.getMethod();
    }
