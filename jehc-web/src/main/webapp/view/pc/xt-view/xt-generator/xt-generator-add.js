...
function addXtGenerator(generatorFlag,type){
	...
	if(generatorFlag == 0){
		...
		if(type == 1){
			xtGeneratorWinAdd.setTitle("快速开发-<font color='red'>单表快速生成-（前端采用Extjs模式开发）</font>");
		}else if(type == 3){
			xtGeneratorWinAdd.setTitle("快速开发-<font color='red'>单表快速生成-（前端采用Bootstrap模式开发）</font>");
			sValue('kf_mode',1);
		}
	}else if(generatorFlag == 1){
		sValue('is_one_to_many',1);
		if(type == 2){
			xtGeneratorWinAdd.setTitle("快速开发-<font color='red'>一对多快速生成-（前端采用Extjs模式开发）</font>");
		}else if(type == 4){
			xtGeneratorWinAdd.setTitle("快速开发-<font color='red'>一对多快速生成-（前端采用Bootstrap模式开发）</font>");
			sValue('kf_mode',1);
		}
