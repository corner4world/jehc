...
//					    jQuery('td:eq('+meta.col+')', meta.row).html(xt_data_dictionary_name); //通过异步渲染数据
			...
			},
			{
				...
				render:function(data, type, row, meta) {
					...
					var btn = '<button class="btn btn-default" onclick=toXtUserinfoDetail("'+data+'")><i class="fa fa-gears"></i>详情</button>';
						btn = btn +'<button class="btn btn-default" onclick=initRoleinfo("'+data+'")><i class="fa fa-user-plus"></i>角色权限</button>';
						btn = btn +'<button class="btn btn-default" onclick=resetXtUserinfoPwd("'+data+'","'+xt_userinfo_realName+'","'+xt_userinfo_name+'")><i class="fa fa-edit"></i>重置密码</button>';
					return btn;
				}
			}
		]
