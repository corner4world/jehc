...
public class Lc_StatusController extends BaseAction{
	@Autowired
	private Lc_StatusService lc_StatusService;
	/**
	* 载入初始化页面
	* @param lc_status 
	* @param request 
	...
	*/
	...
	public ModelAndView loadLcStatus(Lc_Status lc_Status,HttpServletRequest request){
	...
	*/
	...
	public String getLcStatusListByCondition(Lc_Status lc_Status,HttpServletRequest request){
		Map<String, Object> condition = new HashMap<String, Object>();
		commonHPager(condition,request);
		List<Lc_Status> lc_StatusList = lc_StatusService.getLcStatusListByCondition(condition);
		PageInfo<Lc_Status> page = new PageInfo<Lc_Status>(lc_StatusList);
	...
	*/
	...
	public String getLcStatusById(String lc_status_id,HttpServletRequest request){
		Lc_Status lc_Status = lc_StatusService.getLcStatusById(lc_status_id);
	...
	*/
	...
	public String addLcStatus(Lc_Status lc_Status,HttpServletRequest request){
		int i = 0;
		if(null != lc_Status && !"".equals(lc_Status)){
			...
			i=lc_StatusService.addLcStatus(lc_Status);
		}
		if(i>0){
	...
	*/
	...
	public String updateLcStatus(Lc_Status lc_Status,HttpServletRequest request){
		int i = 0;
		if(null != lc_Status && !"".equals(lc_Status)){
			lc_Status.setXt_userinfo_id(getXtUid());
			lc_Status.setLc_status_mtime(CommonUtils.getSimpleDateFormat());
			i=lc_StatusService.updateLcStatus(lc_Status);
		}
		if(i>0){
			return outAudStr(true);
		}else{
	...
	*/
	...
	public String delLcStatus(String lc_status_id,HttpServletRequest request){
		...
		if(null != lc_status_id && !"".equals(lc_status_id)){
			...
			i=lc_StatusService.delLcStatus(condition);
		}
		if(i>0){
			return outAudStr(true);
		}else{
