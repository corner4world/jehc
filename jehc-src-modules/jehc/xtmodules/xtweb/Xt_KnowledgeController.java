...
public class Xt_KnowledgeController extends BaseAction{
	@Autowired
	private Xt_KnowledgeService xt_KnowledgeService;
	/**
	* 载入初始化页面
	* @param xt_knowledge 
	* @param request 
	...
	*/
	...
	public ModelAndView loadXtKnowledge(Xt_Knowledge xt_Knowledge,HttpServletRequest request){
	...
	*/
	...
	public String getXtKnowledgeListByCondition(BaseSearch baseSearch ,String keywords,HttpServletRequest request) throws UnsupportedEncodingException{
		...
		if(!StringUtils.isEmpty(keywords)){
	        ...
	        Solr_Core solrCore = getSolrCoreByUseonlynumbercode("xtknowledge");
		...
		List<Xt_Knowledge> xt_KnowledgeList = xt_KnowledgeService.getXtKnowledgeListByCondition(condition);
		PageInfo<Xt_Knowledge> page = new PageInfo<Xt_Knowledge>(xt_KnowledgeList);
	...
	*/
	...
	public String getXtKnowledgeById(String xt_knowledge_id,HttpServletRequest request){
		Xt_Knowledge xt_Knowledge = xt_KnowledgeService.getXtKnowledgeById(xt_knowledge_id);
	...
	*/
	...
	public String addXtKnowledge(Xt_Knowledge xt_Knowledge,HttpServletRequest request){
		int i = 0;
		...
		if(null != xt_Knowledge && !"".equals(xt_Knowledge)){
			...
			i=xt_KnowledgeService.addXtKnowledge(xt_Knowledge);
		}
		if(i>0){
	...
	*/
	...
	public String updateXtKnowledge(Xt_Knowledge xt_Knowledge,HttpServletRequest request){
		int i = 0;
		String xt_knowledge_content = request.getParameter("xt_knowledge_content");
		if(null != xt_Knowledge && !"".equals(xt_Knowledge)){
			xt_Knowledge.setXt_knowledge_content(xt_knowledge_content);
			i=xt_KnowledgeService.updateXtKnowledge(xt_Knowledge);
		}
		if(i>0){
			return outAudStr(true);
		}else{
	...
	*/
	...
	public String delXtKnowledge(String xt_knowledge_id,HttpServletRequest request){
		...
		if(null != xt_knowledge_id && !"".equals(xt_knowledge_id)){
			...
			i=xt_KnowledgeService.delXtKnowledge(condition);
		}
		if(i>0){
			return outAudStr(true);
		}else{
	...
	*/
    ...
    public static PageSolr solrQueryPage(String baseURL,PageSolr pageSolr) {
        ...
        try {
            ...
            while (iter.hasNext()) {
                ...
                Xt_Knowledge xtKnowledge = new Xt_Knowledge();
    ...
    public String autoXtKnowledgeListByCondition(String keywords,Xt_Knowledge xt_Knowledge,HttpServletRequest request){
