...
public class Xt_SmsController extends BaseAction{
	@Autowired
	private Xt_SmsService xt_SmsService;
	/**
	* 载入初始化页面
	* @param xt_sms 
	* @param request 
	...
	*/
	...
	public ModelAndView loadXtSms(Xt_Sms xt_Sms,HttpServletRequest request){
	...
	*/
	...
	public String getXtSmsListByCondition(BaseSearch baseSearch,HttpServletRequest request){
		...
		List<Xt_Sms> xt_SmsList = xt_SmsService.getXtSmsListByCondition(condition);
		PageInfo<Xt_Sms> page = new PageInfo<Xt_Sms>(xt_SmsList);
	...
	*/
	...
	public String getXtSmsById(String xt_sms_id,HttpServletRequest request){
		Xt_Sms xt_Sms = xt_SmsService.getXtSmsById(xt_sms_id);
	...
	*/
	...
	public String addXtSms(Xt_Sms xt_Sms,HttpServletRequest request){
		int i = 0;
		if(null != xt_Sms && !"".equals(xt_Sms)){
			xt_Sms.setXt_sms_id(UUID.toUUID());
			i=xt_SmsService.addXtSms(xt_Sms);
		}
		if(i>0){
	...
	*/
	...
	public String updateXtSms(Xt_Sms xt_Sms,HttpServletRequest request){
		int i = 0;
		if(null != xt_Sms && !"".equals(xt_Sms)){
			i=xt_SmsService.updateXtSms(xt_Sms);
		}
		if(i>0){
			return outAudStr(true);
		}else{
	...
	*/
	...
	public String delXtSms(String xt_sms_id,HttpServletRequest request){
		...
		if(null != xt_sms_id && !"".equals(xt_sms_id)){
			...
			i=xt_SmsService.delXtSms(condition);
		}
		if(i>0){
			return outAudStr(true);
		}else{
