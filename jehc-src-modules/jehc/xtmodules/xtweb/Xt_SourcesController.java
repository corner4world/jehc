...
public class Xt_SourcesController extends BaseAction{
	@Autowired
	private Xt_SourcesService xt_SourcesService;
	/**
	* 载入初始化页面
	* @param xt_sources 
	* @param request 
	...
	*/
	...
	public ModelAndView loadXtSources(Xt_Sources xt_Sources,HttpServletRequest request){
	...
	*/
	...
	public String getXtSourcesListByCondition(BaseSearch baseSearch,HttpServletRequest request){
		...
		List<Xt_Sources> xt_SourcesList = xt_SourcesService.getXtSourcesListByCondition(condition);
		...
		PageInfo<Xt_Sources> page = new PageInfo<Xt_Sources>(xt_SourcesList);
	...
	*/
	...
	public String getXtSourcesById(String xt_sources_id,HttpServletRequest request){
		Xt_Sources xt_Sources = xt_SourcesService.getXtSourcesById(xt_sources_id);
	...
	*/
	...
	public String addXtSources(Xt_Sources xt_Sources,HttpServletRequest request){
		int i = 0;
		if(null != xt_Sources && !"".equals(xt_Sources)){
			...
			i=xt_SourcesService.addXtSources(xt_Sources);
		}
		if(i>0){
	...
	*/
	...
	public String updateXtSources(Xt_Sources xt_Sources,HttpServletRequest request){
		int i = 0;
		if(null != xt_Sources && !"".equals(xt_Sources)){
			xt_Sources.setXt_sources_mtime(getSimpleDateFormat());
			xt_Sources.setXt_userinfo_id(getXtUid());
			i=xt_SourcesService.updateXtSources(xt_Sources);
		}
		if(i>0){
			return outAudStr(true);
		}else{
	...
	*/
	...
	public String delXtSources(String xt_sources_id,HttpServletRequest request){
		...
		if(null != xt_sources_id && !"".equals(xt_sources_id)){
			...
			i=xt_SourcesService.delXtSources(condition);
		}
		if(i>0){
			return outAudStr(true);
		}else{
	...
	*/
	...
	public ModelAndView toXtSourcesDataView(Xt_Sources xt_Sources,HttpServletRequest request){
