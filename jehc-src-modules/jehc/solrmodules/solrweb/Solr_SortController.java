...
public class Solr_SortController extends BaseAction{
	@Autowired
	private Solr_SortService solr_SortService;
	/**
	* 载入初始化页面
	* @param solr_sort 
	* @param request 
	...
	*/
	...
	public ModelAndView loadSolrSort(Solr_Sort solr_Sort,HttpServletRequest request,Model model){
	...
	*/
	...
	public String getSolrSortListByCondition(Solr_Sort solr_Sort,HttpServletRequest request){
		...
		List<Solr_Sort> solr_SortList = solr_SortService.getSolrSortListByCondition(condition);
		PageInfo<Solr_Sort> page = new PageInfo<Solr_Sort>(solr_SortList);
	...
	*/
	...
	public String getSolrSortById(String solr_sort_id,HttpServletRequest request){
		Solr_Sort solr_Sort = solr_SortService.getSolrSortById(solr_sort_id);
	...
	*/
	...
	public String addSolrSort(Solr_Sort solr_Sort,HttpServletRequest request){
		int i = 0;
		if(null != solr_Sort && !"".equals(solr_Sort)){
			...
			i=solr_SortService.addSolrSort(solr_Sort);
		}
		if(i>0){
	...
	*/
	...
	public String updateSolrSort(Solr_Sort solr_Sort,HttpServletRequest request){
		int i = 0;
		if(null != solr_Sort && !"".equals(solr_Sort)){
			solr_Sort.setSolr_sort_mtime(getSimpleDateFormat());
			solr_Sort.setXt_userinfo_id(getXtUid());
			i=solr_SortService.updateSolrSort(solr_Sort);
		}
		if(i>0){
			return outAudStr(true);
		}else{
	...
	*/
	...
	public String delSolrSort(String solr_sort_id,HttpServletRequest request){
		...
		if(null != solr_sort_id && !"".equals(solr_sort_id)){
			...
			i=solr_SortService.delSolrSort(condition);
		}
		if(i>0){
			return outAudStr(true);
		}else{
