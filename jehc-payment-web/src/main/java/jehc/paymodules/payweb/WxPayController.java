...
import jehc.paymodules.wx.model.WxBank;
...
public class WxPayController {
    ...
    public void init() {
        ...
        wxPayConfigDaoImpl.setMchId("合作者id（商户号）");
        wxPayConfigDaoImpl.setAppid("应用id");
        wxPayConfigDaoImpl.setKeyPublic("转账公钥，转账时必填");
        wxPayConfigDaoImpl.setSecretKey("密钥");
        wxPayConfigDaoImpl.setNotifyUrl("异步回调地址");
        wxPayConfigDaoImpl.setReturnUrl("同步回调地址");
        wxPayConfigDaoImpl.setSignType("签名方式");
        wxPayConfigDaoImpl.setInputCharset("utf-8");
        service = new WxPayServiceImpl(wxPayConfigDaoImpl);
        HttpConfigDb httpConfigDb = new HttpConfigDb();
        //ssl 退款证书相关 不使用可注释
        if(!"ssl 退款证书".equals(KEYSTORE)){
        	httpConfigDb.setKeystore(KEYSTORE);
        	httpConfigDb.setStorePassword(STORE_PASSWORD);
        	httpConfigDb.setPath(true);
        }
        //请求连接池配置
        //最大连接数
