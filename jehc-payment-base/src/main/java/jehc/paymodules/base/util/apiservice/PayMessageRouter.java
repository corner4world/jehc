...
import jehc.paymodules.base.util.LogExceptionHandler;
 ...
 * router
 *   .rule().msgType("MSG_TYPE").event("EVENT").eventKey("EVENT_KEY").content("CONTENT").interceptor(interceptor, ...).handler(handler, ...)
 *   .end().rule().end()
 * ;
 * 将PayMessage交给消息路由器
